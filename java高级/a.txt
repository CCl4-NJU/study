...
public class NIO2FilesDemo {
	public static void main(String[] args) {
		try {
			// 复制文件
			Files.copy(
					Paths.get("src\\chapter01\\NIO2FilesDemo.java"),
					new FileOutputStream("a.txt"));
			// 判断NIO2FilesDemo.java文件是否为隐藏文件
			System.out.println("NIO2FilesDemo.java是否为隐藏文件："
					+ Files.isHidden(Paths
							.get("src/chapter01/NIO2FilesDemo.java")));
			// 一次性读取FilesTest.java文件的所有行
			List<String> lines = Files.readAllLines(Paths.get("src", "chapter01", "NIO2FilesDemo.java"), Charset
					.forName("gbk"));
